...
export default defineComponent({
  setup() {
    ...
    const ensureEditUser = () => {
      ...
      let obj: any = user_data.user_list.find(
        (value) => value.id === user_data.editUser.id
      );
      ...
      if (user_data.selected_data.nickName || user_data.selected_data.role) {
        if (user_data.selected_data.nickName) {
          search_res = user_data.user_list.filter((value) => {
            return (
              value.nickName.indexOf(user_data.selected_data.nickName) !== -1
            );
          });
        }
        if (user_data.selected_data.role) {
          search_res = user_data.selected_data.nickName
            ? search_res
            : user_data.user_list;
          search_res = search_res.filter((value) => {
            return value.role.find(
              (item) => item.role === user_data.selected_data.role
            );
          });
        }
      } else {
