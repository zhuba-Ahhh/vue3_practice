...
export function getBirthday(Day: string = "3-10"): string {
  // 首先要获取到今年的生日
  let birthdayTime: string = moment().format("YYYY-") + Day; // Day "3-10"
  ...
  let MS: number = (moment(birthdayTime) as any) - (moment() as any);
...
function timestampToTime(timestamp: number | null): string {
  ...
  let Y = "";
  let M =
    (date.getMonth() + 1 < 10 ? date.getMonth() : date.getMonth()) + " 个月 ";
  let D = (date.getDate() < 10 ? date.getDate() : date.getDate()) + " 天 ";
  let h = (date.getHours() < 10 ? date.getHours() : date.getHours()) + " 小时 ";
  let m =
    (date.getMinutes() < 10 ? date.getMinutes() : date.getMinutes()) + " 分钟 ";
  let s =
    (date.getSeconds() < 10 ? date.getSeconds() : date.getSeconds()) + " 秒";
  return Y + M + D + h + m + s;
}

/* 时间yyyy-MM-dd HH:mm:ss转为时间戳 */
function timeToTimestamp(time: string): number {
  let timestamp: number = Date.parse(new Date(time).toString());
