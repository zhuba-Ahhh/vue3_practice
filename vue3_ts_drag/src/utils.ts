...
export function getRotatedPoint(
  ...
  return {
    x:
      (point.x - center.x) * Math.cos(rotateToRadian(rotate)) -
      (point.y - center.y) * Math.sin(rotateToRadian(rotate)) +
      center.x,
    y:
      (point.x - center.x) * Math.sin(rotateToRadian(rotate)) +
      (point.y - center.y) * Math.cos(rotateToRadian(rotate)) +
      center.y,
  };
}
